...
int main() {
	...
	vector<double> truth_table[1<<input_ports];
	vector<double> input_pattern[1<<input_ports];

	fstream f("inp_digit_recognition");

	// int A[100];

	for(int i=0;i<(1<<input_ports);i++) {
	...
	while(1) {
		...
		for(int i=0;i<(1<<input_ports);i++) {
			//  cout<<i<<" "<<error<<endl;
			error=ffnn.train(truth_table[i],input_pattern[i],learn_rate,momentum);
			// cout<<i<<" "<<error<<endl;
			// cout<<cnt<<" "<<error<<endl;
			cnt++;
			if(error > 0.01)
	...
	vector<double> input_pattern_given(input_ports);
	while(1) {
		cout<<"----------------------------------\n\n";
		cout<<"Fill the state of LED's \n\n";

		for(int i=0;i<7;i++) {
			cout<<"Enter the state of LED "<<i<<" : ";
			cin>>input_pattern_given[i];
			cout<<endl;
		}

		ffnn.propagate(input_pattern_given);

		bool ok=false;

		cout<<"----------------------------------\n\n";

		int dig=-1;double val=0;

		cout<<"Output Layer output:\n";
		for(int j=0;j<output_ports;j++) {
			val=ffnn.output_layer.neurons[j].output;
			cout<<ffnn.output_layer.neurons[j].output<<"\n";
			if(val>0.9)
				dig=j;
		}
		...
		if(dig==-1)
			cout<<"The LED's don't represent any digit\n";
		else
			cout<<"The LED shows the digit : "<<dig<<endl;
		cout<<endl;
	}
